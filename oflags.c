...
void	o_minus(unsigned long long arg, int len, char *in)
{
	...
	if (tmplen <= 0)
	{
		...
		tmplen = g_f->wid - len + (!arg && g_f->dot && !g_f->pre) - (g_f->of && arg) - (!arg && g_f->of && g_f->dot && !g_f->pre);
		addcharn(' ', tmplen);
	}
	else
	{
...
void	o_precision(unsigned long long arg, int len, char *in)
{
	...
	if (g_f->pre >= len)
		tmplen = g_f->wid - g_f->pre + (!arg && g_f->dot && !g_f->pre) - (g_f->of && g_f->dot && !g_f->pre);
	else
		tmplen = g_f->wid - len + (!arg && g_f->dot && !g_f->pre) - (g_f->of && g_f->dot && !g_f->pre && !arg) - (g_f->of && arg);
	if (tmplen <= 0)
	{
		if (g_f->of)
			g_vector = ft_vector(g_vector, "0", 5, 1);
