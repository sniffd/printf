...
void	x_long(va_list ap)
{
	...
	in = ft_strtolower(ft_itoa_base(arg, 16));
	len = (int)ft_strlen(in);
	tmplen = g_f->wid - (len * (!(!arg && g_f->dot && !g_f->pre) && (len > g_f->wid || !g_f->dot))) + (!arg && g_f->dot && !g_f->pre) - ((g_f->okt || g_f->f == 'p') && arg) * 2 - (!arg && (g_f->okt || g_f->f == 'p') && g_f->dot && !g_f->pre) - (g_f->pre - 1 * (g_f->pre - 1 > 0)) * (g_f->pre > len + ((g_f->okt || g_f->f == 'p') && arg) * 2);
	if (tmplen <= 0)
	{
		tmplen = g_f->pre - len;
		oktotorp(arg);
